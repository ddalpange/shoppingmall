...
export default combineReducers({
  productItems: (productItems = defaultState().productItems, action) => {
    switch (action.type) {
      default:
        return productItems;
    }
  },
  coupons: (coupons = defaultState().coupons, action) => {
    switch (action.type) {
      default:
        return coupons;
    }
  },
  wishlist: (wishlist = defaultState().wishlist, action) => {
    const result = wishlist.slice();

    switch (action.type) {
      case "PUSH_WISHLIST":
        if (wishlist.length >= 3) {
          window.alert("장바구니는 3개까지 담을 수 있습니다.");
        } else {
          result.push(action.product);
        }
        return result;

      case "POP_WISHLIST":
        result.splice(
          result.findIndex(wish => wish.id === action.product.id),
          1
        );
        return result;
      default:
